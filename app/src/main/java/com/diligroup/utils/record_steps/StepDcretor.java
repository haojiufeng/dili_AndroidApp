...
//计算x y z 3轴 的算术平均数   知乎上没有同步代码块
    ...
    private void preStep() {
        if (CountTimeState == 0) {// 0-准备计时   1-计时中   2-正常计步中
            // 开启计时器
            time = new TimeCount(duration, 700);
            time.start();
            CountTimeState = 1;
//            Toast.makeText(context, "开启计时器",Toast.LENGTH_LONG).show();

        } else if (CountTimeState == 1) {
            TEMP_STEP++;
//            Toast.makeText(context,"计步中 TEMP_STEP:" + TEMP_STEP,Toast.LENGTH_LONG).show();
...
//       LogUtils.e(TAG, "oldValue:" + oldValue);
            ...
            TimerTask task = new TimerTask() {
                public void run() {
                    if (lastStep == CURRENT_SETP) {
                        ...
                        lastStep = -1;
                        TEMP_STEP = 0;
                        LogUtils.e(TAG, "停止计步：" + CURRENT_SETP);
                    } else {
                        lastStep = CURRENT_SETP;
                    }
                }
            };
            timer.schedule(task, 0, 2000);// hjf
            CountTimeState = 2;
        }
